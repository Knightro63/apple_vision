# apple\_vision\_object\_tracking

[![Pub Version](https://img.shields.io/pub/v/apple_vision_object_tracking)](https://pub.dev/packages/apple_vision_object_tracking)
...
Apple Vision Selfie Detection is a Flutter plugin that enables Flutter apps to use [Apple Vision Object Detection](https://developer.apple.com/documentation/vision/vntrackobjectrequest).
...
**MacOS**
 - Minimum osx Deployment Target: 10.14
 - Xcode 13 or newer
 - Swift 5
...
```dart
  ...
  List<Widget> showRects(){
    if(objectData == null || objectData!.objects.isEmpty) return [];
    List<Widget> widgets = [];
    ...
    for(int i = 0; i < objectData!.objects.length; i++){
      //if(objectData!.objects[i]. confidence > 0.5){
        widgets.add(
          Positioned(
            top: objectData!.objects[i].top,
            left: objectData!.objects[i].left,
            child: Container(
              width: objectData!.objects[i].width*imageSize.width,
              height: objectData!.objects[i].height*imageSize.height,
...
Find the example for this API [here](https://github.com/Knightro63/apple_vision/tree/main/packages/apple_vision_object_tracking/example/lib/main.dart).
